'''
A handmade hashing algorithm courtesy of Sarah Cross.
'''
...
np.random.seed(seed)
HVars = [[np.random.randint(0, 2)
          for ii in range(sectionSize)] for i in range(5)]
...
def hash(inputStuff, HVars=HVars):
    # "".join([chr(np.random.randint(97, 97+25)) for i in range(64)]) for testing
    wordHash = copy.deepcopy(inputStuff)
    ...
    wordHash = np.array(wordHash).reshape(
        int(len(wordHash) / sectionSize), sectionSize)
    ...
    for i in range(STEPS):
        # ORs 4 random words.
        results.append(
            OR(*[wordHash[np.random.randint(len(wordHash))] for ii in range(4)])[1:] + [0])
    # return results
    ...
    for i in range(1000):
        ...
        copyHVars[2] = OR(
            NOT(*results, SHIFT(SHIFT(AND(NOT(*results, HVars[2]))))))
        copyHVars[0] = SHIFT(
            OR(NOT(SHIFT(SHIFT(AND(SHIFT(OR(*results, HVars[3]))))))))
...
def hashTag(input1):
    '''
    This returns a hashed value for an input. 
    '''
